...
class NewCall:
    ...
    def _performCall(self, parameters):
        if self.mode == 'PPMS API':
            ...
            if not response.status_code == 200:
                raise Errors.APIError(True, False, msg='Error ' + str(
                    response.status_code) + ': API didn\'t return a proper response')
    ...
    def _sendToProxy(self, param_dict):
        ...
        try:
            proxy_socket.connect(
                (self.SYSTEMoptions.getValue('proxy_address'), int(self.SYSTEMoptions.getValue('API_port'))))
    ...
    def getBookedSessionsPeriod(self, start_date: datetime.date, end_date: datetime.date, PPMS_facity_id: str) -> list:
        parameters = {
            'action': 'Report70',
    ...
    def getTodaysBookings(self, PPMS_facility_id, PPMS_name=None, filter=True,
